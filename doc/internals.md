...
```ocaml
(* In utop #require "current_incr" *)
open Current_incr

let _ = 
    let x = Current_incr.var 3 in
    let y = Current_incr.map (fun x -> x * 2) (Current_incr.of_var x) in
    let display = Current_incr.map (Printf.printf "y is now %d\n") y in
    display
```
...
let x = 3
let y = x * 2
let display = Printf.printf "y is now %d\n" y
```

However, we can change `x` and recompute everything that depends on it:

```
utop # Current_incr.change x 21;;
utop # Current_incr.propagate ();;
y is now 42
```
...
utop # test (Term.return 21);;
- : int Current_term__.Output.t = Ok 42

utop # test (Term.fail "Crashed");;
- : int Current_term__.Output.t = Error (`Msg "Crashed")

utop # test (Term.active `Running);;
...
```
utop # let result = Term.return ~label:"build result" "image1" |> await_approval;;
utop # Term.Executor.run result |> Current_incr.observe;;
- : string Current_term__.Output.t = Error (`Active `Ready)

utop # Current_incr.change approved true;;
utop # Current_incr.propagate ();;
utop # Term.Executor.run result |> Current_incr.observe;;
...
```
utop # Format.printf "%a@." Term.Analysis.pp result;;
